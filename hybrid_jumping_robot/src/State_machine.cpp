...
State_machine::State_machine() : input("0"), state_change(true) {
  ...
  state_machine_output = nh.advertise<std_msgs::Bool>("/HJC/State_machine/Output", 1);
...
void State_machine::callback_input(const std_msgs::String &data) {
  ...
  std_msgs::Bool msg;
  msg.data = input == "f";
  state_machine_output.publish(msg);
}
